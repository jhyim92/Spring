...
//	메세지 발송
	...
	public void handleTextMessage(WebSocketSession session, TextMessage message) {
		System.out.println("SocketHandler handleTextMessage start...3번");
		...
		for (String key : sessionMap.keySet()) {
			...
			try {
				...
				System.out.println("SocketHandler handleTextMessage Exception->" + e.getMessage());
...
//	10. unused : 사용하지 않는 코드 관련 경고 억제
	...
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {
		System.out.println("SocketHandler afterConnectionEstablished start...2번");
//		웹소켓 연결이 되면 동작
		...
		System.out.println("SocketHandler afterConnectionEstablished session.getId()->" + session.getId());
...
//	handleTextMessage 메소드에 JSON파일이 들어오면 파싱해주는 함수를 추가
	private static JSONObject jsonToObjectParser(String jsonStr) {
		System.out.println("SocketHandler static JSONObject jsonToObjectParser start...4번");
		JSONParser parser = new JSONParser();
		JSONObject jsonObj = null;
		try {
			jsonObj = (JSONObject) parser.parse(jsonStr);
		} catch (Exception e) {
			System.out.println("SocketHandler JSONObject jsonToObjectParser Exception->" + e.getMessage());
		}
		return jsonObj;
	}
