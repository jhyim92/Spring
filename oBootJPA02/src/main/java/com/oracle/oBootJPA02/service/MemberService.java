...
//	회원가입
	public Member join(Member member) {
		...
		System.out.println("MemberService join member.getTeamname()->" + member.getTeamname());
...
//	회원상세조회
	public Optional<Member> findByMember(Long memberId) {
		...
		System.out.println("MemberService findByMember member.get().getName()+ member.get().getTeam().getName()->"
				+ member.get().getName() + member.get().getTeam().getName());
		return member;
	}
